angular.module("altairApp").controller("issuesCtrl", [
  "$rootScope",
  "$scope",
  "$stateParams",
  "issues_data",
  ...
  function ($rootScope, $scope, $stateParams, issues_data, utils, variables) {
    ($rootScope.pageHeadingActive = !0),
      $scope.$on("$destroy", function () {
        $rootScope.pageHeadingActive = !1;
      }),
      ($scope.project_name = "Altair"),
      ($scope.issues = issues_data),
      ($scope.issue_details = utils.findByItemId(
        $scope.issues,
        $stateParams.issueId
      )),
      ($scope.newTask = {
        ...
        if ($(element).closest($ts_issues).length) {
          ...
          $ts_issues
            ...
            .on("pagerComplete", function (e, filter) {
              "undefined" != typeof selectizeObj &&
                selectizeObj.data("selectize") &&
                ((selectizePage = selectizeObj[0].selectize),
                selectizePage.setValue(
                  $("select.ts_gotoPage option:selected").index() + 1,
                  !1
                ));
            }),
            createPageSelectize(),
